[English](README.md) · **简体中文** · [繁體中文](README.zh-TW.md) · [Français](README.fr.md) · [Русский](README.ru.md) · [日本語](README.ja.md) · [Tiếng Việt](README.vi.md)
...
| `src/i18n/` | 七种语言的文案包 + 语言判定（cookie → Accept-Language → 英语） |
...
| `src/styles/shell.css` | 应用外壳：顶栏、侧栏、状态栏、语言下拉 |
...
| `src/components/LanguageSwitcher.tsx` | 语言下拉，就在主题切换按钮的左边 |
| `src/components/BrandMark.tsx` | 品牌标记（等距立方体 + 上行信号），见下 |
| `src/lib/theme.ts` | 主题持久化（跟随系统 / 浅色 / 深色）+ 首屏防闪脚本 |

四个刻意的取舍：

- **主题在首屏之前定好**。`themeBootstrapScript` 内联进 `<head>`，在第一次绘制前就把
  `data-theme` 打到 `<html>` 上，所以不会有一闪的白底。
- **主题默认跟随系统**。存起来的是**偏好**（`system` / `light` / `dark`），只有 `<html>` 上的
  `data-theme` 才是解析后的颜色 —— 两者必须分开，否则「跟随系统」这一档没地方表示。
...
界面提供**简体中文、繁體中文、English、Français、Русский、日本語、Tiếng Việt** 七种语言。
三条值得知道的决定：

- **语言在服务端定**：`mxds.lang` cookie（用户显式选过的）→ `Accept-Language`（也就是跟随系统）
  → 兜底英语。必须在服务端算：`<html lang>` 和首帧文案都得是对的，在客户端读
  `navigator.language` 会让每次打开都先闪一下另一种语言。
- **每份语言包都拿英语那份做类型校验**。`src/i18n/messages/en.ts` 定义键集合，其余六份声明成
  `Messages`，于是少一个键或拼错会在 `npm run typecheck` 时报错，而不是在界面上渲染出一个裸键。
  占位符是 `{name}`；文案里的行内强调写成 `**粗**` / `` `等宽` `` 由 `<RichText>` 渲染 ——
  这样语言包里没有任何标签结构，翻译的人不必碰 JSX。
- **API 的错误消息是消息键，不是成品文案**。路由处理器抛的是 `api.node.duplicate` 这样的键，
  由 `route()` 那层包装（`src/lib/api-route.ts`）按**发起这次请求的人的语言**翻一次。
  zod 的校验消息同理，所以 `src/lib/validation.ts` 仍然是一个零依赖的纯数据模块。

语言下拉就放在主题切换按钮的左边 —— 外壳顶栏和首页顶栏各一份。手机端的首页顶栏不显示它：
那里顶栏的职责是 LOGO、项目名和 GitHub 图标（见下），而登录页在角上另有一份，
所以「系统语言没被认出来」不会变成一条死路。
...
**顶栏按优先级砍项，而不是卡在一个写死的断点上。** 项目名是固定的 23 个字符，但
「登录 / 注册」在法语里比中文宽近一倍，所以任何写死的断点都会在某几种语言上砍得过早或过晚。
`src/components/LandingBarFit.tsx` 改成量出来：装不下时，主题切换和登录/注册按钮**一起**让位，
保证 LOGO、完整的项目名和 GitHub 图标留在原处。电脑端永远不会砍任何一项。
